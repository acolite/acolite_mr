...
def image_test(file, listpan=False):
    ...
    ## return file if nc file
    if '.nc' in file:
        import glob
        fdir=os.path.dirname(file)
        fbase=os.path.basename(file).split('.')[0]
        metafile = '{}/{}.{}'.format(fdir,fbase,'xml')
        print(metafile)
        if os.path.exists(metafile):
            mfile = glob.glob('{}/{}.{}'.format(fdir,fbase,'xml'))#[0]
        else:
            print('metafile {} not found'.format(metafile))
            exit(1)

        if type(file) is not list: file = [file]
        if type(mfile) is not list: mfile = [mfile]
        print(file)
        if listpan:
                return(file,mfile,[None],[None])
        else:
                return(file,mfile)
